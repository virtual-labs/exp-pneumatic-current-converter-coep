...
//PC_CALIBRATION.PC_calibrationFun = function(lowerSpLevel, higherSpLevel, PColdreading, PCarr_actualVal, PCarr_stdVal) {
	 ...
	 PC_calibrationFun_Reverse = function(i2pType, lowerSpLevel, higherSpLevel, PColdreading, PCarr_actualVal, PCarr_stdVal, lowerOutputLevel, higherOutputLevel) {
...
//						alertify.alert("Correct Answer is: " + PC_ZeroError.toFixed(3));
	...
	$('#PC_calibrateNeeded').on('click', function() {
		...
		// var range = $("#zeroerror").val();
		 var flag = 0;
		PCZero_CalibartionRev(range, flag);
	
	//	 PC_CheckLinearitySectionRev();
		

	});
	
	
	var PCZero_CalibartionRev = function(range, flag){
...
//						 console.log(n);
						...
						//console.log(ZeroValue);
					     ...
					     if(range == n)
							...
							PCarr_actualValSorted.splice(0, 1, parseFloat(20));
							///$('#minus').hide();
							//$('#plus').hide();
							  $("#ZeroangleVal").val(PCarr_actualValSorted[0]);
...
//								alertify.alert("Zero Adjusted Successfully!!!");
	...
	var PCSpan_ClaibrationNeededRev = function() {
		var PC_calibrateNeeded = '';
		PC_calibrateNeeded += '<div class="col-md-12" >'
...
//				console.log(n);
				...
				// $('#minus').show();
				 ...
				 for( var i= 1; i<=(PCarr_actualValSorted.length-2); i++ )
					...
					if((-range) == n)
					{ 
						PCarr_actualValSorted.splice((ArrayLeng - 1), 1, parseFloat(4));
						PCarr_actualValSorted.splice(0, 1, parseFloat(ZeroValue));
						$('#Spanminus').hide();
...
//					 console.log(n);
	...
	var PC_calibrate_Top_Linearity_NeededRev = function ()
	{
...
//			 LinearityTopminus  LinearityTopplus
						...
						for( var i= 1; i<=(PCarr_actualValSorted.length-2); i++ )
						 ...
						 if(defError == n)
							...
							if(positiveVal.length != 0){
								...
								PC_calibrate_Bottom_Linearity_NeededRev();
								$("#LinearityBottomKnotch").prop("hidden", false);
							}else{
								
								PC_calibrate_Both_Linearity_NeededRev();
	...
	var PC_calibrate_Bottom_Linearity_NeededRev = function ()
	{
//		$("#ZeroErrorKnotch").hide();
//		$("#SpanErrorKnotch").hide();
...
//				     LinearityBottomminus   LinearityBottomplus	
	...
	var PC_calibrate_Both_Linearity_NeededRev = function(){
		$("#ZeroErrorKnotch").hide();
		$("#SpanErrorKnotch").hide();
...
//			//console.log("Min Diffrence "+ minDiff);
							 ...
							 if((defError) == n)
								{
								 ...
								 $('#LinearitySectionBothKnockData').hide();
									...
									PC_updatedDataTableRev(PColdreadingSorted,PCarr_actualValSorted);
	...
	var PC_CheckLinearitySectionRev = function()
	{
		...
		$("#LinearityKnotch").prop("disabled", false);
			...
			if(negativeVal.length != 0 || (positiveVal.length != 0 && negativeVal.length != 0)){
				$("#LinearityTopKnotch").prop("hidden", false);
				PC_calibrate_Top_Linearity_NeededRev();
	...
	var PC_updatedDataTableRev = function(PColdreadingSorted,PCarr_actualValSorted)
	{
...
//		drawGraphRev(PColdreading, PCarr_actualVal)
...
//		console.log(ExpTrackData);
	...
	function drawGraphRev(PColdreadingSorted, PCarr_actualValSorted){
		var OldValue = [];
		var StdValue = [];
